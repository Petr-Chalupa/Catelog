...
function mapCSFDToTitle(data: any): TitleCreate {
    ...
    return {
        ...
        genres: data.genres ? data.genres.map((g: CSFDGenres) => CSFD_GENRE_MAP[g]).filter(Boolean) : [],
...
export async function searchCSFD(query: string, details: boolean): Promise<TitleCreate[]> {
    const res = await csfd.search(query);

    const searchResults = [...(res.movies || []), ...(res.tvSeries || [])];

    let finalData: TitleCreate[] = [];
...
export async function searchCSFDById(csfdId: string): Promise<TitleCreate | null> {
